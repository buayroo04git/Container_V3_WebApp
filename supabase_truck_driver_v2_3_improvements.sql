...
-- รองรับ Idempotent (รันซ้ำได้ปลอดภัย 100%)
-- ==============================================================================
...
-- ------------------------------------------------------------------------------
-- 2. Ensure Tables Exist with Core Columns
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- CHECK status on driver_records
    ...
    -- CHECK status on truck_operations
    ...
    -- CHECK operation_type on truck_operations
    ...
    -- CHECK status on truck_maintenance_records
    ...
    -- CHECK status on driver_leave_records
...
-- ------------------------------------------------------------------------------
-- 4. Foreign Key Constraints (Cascade Updates to prevent orphaned records)
-- ------------------------------------------------------------------------------
DO $$
BEGIN
    -- Foreign Key: truck_operations(truck_no) -> truck_records(truck_no)
    ...
    -- Foreign Key: truck_operations(driver_name) -> driver_records(driver_name)
    ...
    -- Foreign Key: truck_maintenance_records(truck_no) -> truck_records(truck_no)
    ...
    -- Foreign Key: driver_leave_records(driver_name) -> driver_records(driver_name)
...
-- ------------------------------------------------------------------------------
-- 5. Auto-compute Maintenance Cost Trigger
-- ------------------------------------------------------------------------------
...
CREATE TRIGGER trg_driver_leave_updated_at
    BEFORE UPDATE ON public.driver_leave_records
    FOR EACH ROW EXECUTE FUNCTION public.trigger_set_updated_at();
...
-- ------------------------------------------------------------------------------
-- 8. Enhanced Atomic RPCs with created_by Parameter
-- ------------------------------------------------------------------------------

-- 8.1 มอบหมายคนขับประจำรถ (Assign Driver RPC)
...
-- 8.2 ปลดคนขับออกจากรถ (Unassign Driver RPC)
