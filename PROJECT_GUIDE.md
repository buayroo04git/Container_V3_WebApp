...
## 📜 13. บันทึกการแก้ไขระบบและสถาปัตยกรรมใหม่ (Revision History & Architecture Log)

### 🗓️ วันที่ 28 สิงหาคม 2026: ระบบรอบงานมาตรฐานสากล, การตรวจจับงานข้ามรอบ และหน้ารายงานกระทบยอดตู้ DG

#### 1. มาตรฐานรูปแบบรอบงานสากล (Batch Standardization: `DD - DD MMM YYYY`)
- **รูปแบบมาตรฐานเดียวทั้งระบบ:** `[วันเริ่มต้น] - [วันสิ้นสุด] [เดือนย่อภาษาอังกฤษ] [ปี ค.ศ.]` เช่น `01 - 15 APR 2026`, `16 - 31 MAY 2026`, `01 - 15 AUG 2026`
- **ฟังก์ชันแปลงรอบงาน:** พัฒนา `cleanBatchName` / `standardizeBatchName` และ `parseBatchPeriod` ใน [`matchingLogic.js`](file:///C:/Users/AMD/Desktop/Container_V3_WebApp/src/utils/matchingLogic.js) ตัดคำนำหน้า/ต่อท้าย (`วางบิลDG`, `TSAW`, `LINE_ALBUM_`, `Rev`, `แก้ของ`) และแปลงชื่อโฟลเดอร์ เช่น `Apr1-15 501`, `505 1-15Apr`, `ใบงาน69Aug1-15` ให้เป็นมาตรฐานทันที
- **การแยกความเป็นเอกเทศ (Batch Independence):**
  - ตาราง `container_records` (ใบวางบิล): ยึดรอบของใบวางบิล (`billing_batch`)
  - ตาราง `job_sheets` (ใบงาน) และ `job_sheet_items`: ยึดรอบของใบงาน (`batch_name`)
  - หน้า `OCR Container History`: แสดงรอบของใบงานเป็นหลัก และเก็บ `billing_batch` แยกคู่ขนานเพื่อใช้เปรียบเทียบ

#### 2. ลอจิกการจำแนกงานข้ามรอบ (Cross-Batch Evaluation Engine)
- พัฒนา `evaluateCrossBatchRelation(sheetBatch, billingBatch, jobDate)` คำนวณ Period Index จากปี + เดือน + ช่วงครึ่งเดือน (1-15 / 16-สิ้นเดือน)
- จำแนกออกเป็น 3 สถานะ:
  - 🟢 **`IN_CYCLE` (ตรงรอบ):** รอบใบงานและใบวางบิลตรงกัน
  - 🟠 **`PREV_PERIOD` (ตกจากรอบก่อน / งานตกเบิก):** รอบวางบิลเกิดขึ้นก่อนรอบใบงาน $\rightarrow$ แสดงป้ายสีส้มแอมเบอร์ `[ ↩️ ตกจาก: DD - DD MMM YYYY ]`
  - 🟣 **`NEXT_PERIOD` (ข้ามไปรอบหน้า / งานยกยอด):** รอบวางบิลเกิดขึ้นหลังรอบใบงาน $\rightarrow$ แสดงป้ายสีม่วงอินดิโก้ `[ ↪️ ยกไป: DD - DD MMM YYYY ]`

#### 3. หน้ารายงานสรุปตู้ DG ประจำเดือนและกระทบยอด (DG Monthly Reconciliation View)
- สร้างหน้าวิวใหม่ [`DgMonthlyReconciliationView.jsx`](file:///C:/Users/AMD/Desktop/Container_V3_WebApp/src/views/DgMonthlyReconciliationView.jsx) ถอดแบบโครงสร้างตาราง Multi-level Header สีเขียวจากแบบฟอร์มบัญชีจริง (`21826.png`)
- ลงทะเบียนในแถบเมนูหลัก **"แดชบอร์ด"** $\rightarrow$ **"📑 ตู้ DG ประจำเดือน"**
- **คอลัมน์เจ้าของรถ:** ดึงข้อมูลเจ้าของรถจากฟิลด์ `owner` ในตาราง `truck_records` (เมนูรถ) โดยตรง
- **สมการกระทบยอด 4 เสาหลัก (Reconciliation Formula):**
  $$\mathbf{รวมตู้ใบงาน วางบิลเดือนนี้} = \mathbf{(1) - (2) + (3) - (4)} = \mathbf{รวมจำนวนตู้วางบิล}$$
  - **`(1)` รวมจำนวนตู้ใบงาน:** ตู้ทั้งหมดที่คนขับส่งมาในใบงานรอบเดือนนี้
  - **`(2)` จำนวนตู้ใบงานที่วางบิลแล้วในเดือนก่อนหน้า:** คนขับเพิ่งส่งใบงานรอบนี้ แต่วางบิลไปแล้วในเดือนก่อน $\rightarrow$ **หักออก (-)** แสดงเป็น `(X)`
  - **`(3)` จำนวนตู้วางบิลแล้วจากใบงานเดือนหน้า:** วางบิลเดือนนี้แล้ว แต่ใบงานจะมาเดือนหน้า $\rightarrow$ **บวกเข้า (+)** แสดงเป็น `+X`
  - **`(4)` จำนวนตู้ค้างวางบิลจากใบงานยกไปเดือนหน้า:** คนขับส่งใบงานมาแล้ว แต่วางบิลไม่ทัน/ยังไม่ได้วางบิล/ยกยอดไปวางบิลเดือนหน้า $\rightarrow$ **หักออก (-)** แสดงเป็น `(X)`
- **แถวสรุปผลรวม (Grand Total Row):** คำนวณ SUM ทุกคอลัมน์อัตโนมัติ พร้อมตรวจเช็คความสมดุล (`✓ ตรงกัน 100%`)
- **การส่งออกไฟล์:** รองรับการ Export ตารางรายงานกระทบยอดเป็นไฟล์ `.xlsx` (Excel) ทันที
